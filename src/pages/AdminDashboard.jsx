...
function AdminDashboard() {
  ...
  if (loading) return <StateScreen icon={<RefreshCw className="loading-icon" size={28} />} title="Loading Dayflow..." message="Fetching HR data from Firebase." />;
  ...
  return <div className="hr-layout">
    <aside className="hr-sidebar"><div className="hr-brand"><div className="brand-logo">D</div><div><h2>Dayflow</h2><span>HR Management</span></div></div><nav className="hr-nav"><p className="nav-label">MAIN</p>{sections.map(([label, Icon]) => <button key={label} className={activeSection === label ? "nav-item active" : "nav-item"} onClick={() => setActiveSection(label)}><Icon size={19} />{label}{label === "Leave Requests" && pendingLeaves > 0 && <span className="nav-badge">{pendingLeaves}</span>}</button>)}</nav><div className="sidebar-bottom"><div className="admin-mini-profile"><div className="admin-avatar">HR</div><div><strong>HR Admin</strong><span>Administrator</span></div></div><button className="logout-button" onClick={handleLogout}><LogOut size={18} />Logout</button></div></aside>
